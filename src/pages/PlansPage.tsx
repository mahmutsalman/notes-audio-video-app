...
import type { RecordingPlanWithContext, DurationPlanWithContext, UpdateRecordingPlan, UpdateDurationPlan, CalendarTodo } from '../types';
...
export default function PlansPage() {
  ...
  const todosForDay = useMemo(
    () => todos.filter(t => t.plan_date === selectedDate).sort((a, b) => a.sort_order - b.sort_order || a.created_at.localeCompare(b.created_at)),
    [todos, selectedDate]
  );
  ...
  const handleAddTodo = useCallback(async () => {
    ...
    if (!text) return;
    setNewTodoText('');
    const created = await window.electronAPI.calendarTodos.create({ plan_date: selectedDate, text });
    setTodos(prev => [...prev, created]);
  }, [newTodoText, selectedDate]);

  const handleTodoToggle = async (todo: CalendarTodo) => {
    const updated = await window.electronAPI.calendarTodos.update(todo.id, { completed: todo.completed ? 0 : 1 });
    setTodos(prev => prev.map(t => t.id === todo.id ? updated : t));
  };

  const handleTodoDelete = async (todo: CalendarTodo) => {
    await window.electronAPI.calendarTodos.delete(todo.id);
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wide">Todos</span>
              {todosForDay.length > 0 && (
                <span className="text-xs text-gray-400 dark:text-gray-500">
                  {todosForDay.filter(t => t.completed).length}/{todosForDay.length} done
                </span>
              )}
            </div>
            ...
            <div className="space-y-1 mb-2">
              {todosForDay.filter(t => !t.completed).map(todo => (
                <div key={todo.id} className="group flex items-start gap-2 py-1 px-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <button
                    onClick={() => handleTodoToggle(todo)}
                    className="mt-0.5 flex-shrink-0 w-4 h-4 rounded border border-gray-300 dark:border-gray-600 hover:border-amber-400 transition-all duration-150"
                  />
                  <span className="flex-1 text-sm leading-snug text-gray-700 dark:text-gray-200">
                    {todo.text}
                  </span>
                  <button
            ...
            <div className="flex items-center gap-2">
              <div className="flex-shrink-0 w-4 h-4 rounded border border-dashed border-amber-300 dark:border-amber-700" />
              <input
                ref={todoInputRef}
                value={newTodoText}
                ...
                className="flex-1 text-sm bg-transparent outline-none text-gray-600 dark:text-gray-400 placeholder-gray-400 dark:placeholder-gray-600 border-b border-transparent focus:border-amber-300 dark:focus:border-amber-700 py-0.5 transition-colors"
              />
              {newTodoText.trim() && (
                <button
                  onClick={handleAddTodo}
                  className="flex-shrink-0 text-xs text-amber-500 hover:text-amber-600 dark:text-amber-400 font-medium transition-colors"
                >
                  Add
                </button>
              )}
            ...
            {/* Completed todos */}
            {todosForDay.some(t => t.completed) && (
              <div className="mt-3">
                <span className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide">Completed</span>
                <div className="space-y-1 mt-1">
                  {todosForDay.filter(t => t.completed).map(todo => (
                    <div key={todo.id} className="group flex items-start gap-2 py-1 px-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <button
                        onClick={() => handleTodoToggle(todo)}
                        className="mt-0.5 flex-shrink-0 w-4 h-4 rounded border bg-emerald-500 border-emerald-500 text-white transition-all duration-150"
                      >
                        <svg className="w-3 h-3 m-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                        </svg>
                      </button>
                      <span className="flex-1 text-sm leading-snug line-through text-gray-400 dark:text-gray-500">
                        {todo.text}
                      </span>
                      <button
                        onClick={() => handleTodoDelete(todo)}
          ...
          {/* ── Recording plans section ────────────────────── */}
          {(plansForDay.length > 0 || loading) && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">From Recordings</span>
                <span className="text-xs text-gray-400 dark:text-gray-500">
